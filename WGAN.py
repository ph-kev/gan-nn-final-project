...
# Keep track of loss for discriminator and generator 
dis_loss_arr = []
gen_loss_arr = []
dis_real_arr = []
dis_fake_arr = []
...
for curr_epoch in range(1, num_epoch + 1):
    ...
    for batch_real_image, _ in data_loader:
        ...
        for i in range(0, 5):
            ...
            real_probs = discriminator(batch_real_image)
            ...
            dis_loss = wgan_discriminator_loss(real_probs, fake_probs)
            dis_loss.backward()
            dis_optimizer.step()
...
with open('saved_losses/wgan_dis_loss_arr.pkl', 'wb') as f:
    pickle.dump(dis_loss_arr, f)

with open('saved_losses/wgan_gen_loss_arr.pkl', 'wb') as f:
    pickle.dump(gen_loss_arr, f)

with open('saved_losses/wgan_dis_real_arr.pkl', 'wb') as f:
    pickle.dump(dis_real_arr, f)

with open('saved_losses/wgan_dis_fake_arr.pkl', 'wb') as f:
